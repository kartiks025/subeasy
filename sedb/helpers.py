...
def get_assign_home(request, sec_user_id, assign_id):
    ...
    for prob in problems:
        prob_list.append({'prob_id': prob.problem_id, 'prob_num': prob.problem_no})
    deadline = assignment.deadline
    context = {'new_assign': False, 'assign': model_to_dict(assignment),
               'deadline': model_to_dict(deadline), 'prob_list': prob_list}
...
def get_assignments(request, sec_user_id):
    ...
    if sec_user.role == "Instructor" or sec_user.role == "TA":
        ...
        assign = Assignment.objects.filter(sec=sec_user.sec, visibility=True,
                                           publish_time__lt=datetime.datetime.now()).order_by('-publish_time')
    assignments = []
    for a in assign:
...
def get_assign_prob(request, sec_user_id, assign_id, prob_id):
    ...
    testcases = [{'id': a.id, 'infile_name': a.infile_name, 'outfile_name': a.outfile_name, 'num': a.testcase_no,
                  'marks': a.marks, 'visibility': a.visibility} for a in test]
    # print(testcases)
    return JsonResponse({
        'new_prob': False,
...
def get_user_assign_prob(request, sec_user_id, assign_id, prob_id):
    ...
    try:
        final = UserSubmissions.objects.get(user_id=request.session['user_id'], problem_id=prob_id).final_submission_no
        sub_file_name = Submission.objects.get(user_id=request.session['user_id'], problem_id=prob_id,
                                               sub_no=final).sub_file_name
    except:
        sub_file_name = ""
    return JsonResponse({
        'new_prob': False,
...
def submit_problem(request, sec_user_id, assign_id, prob_id):
    try:
        ...
        if assignment.deadline.soft_deadline > datetime.datetime.now():
            ...
            message = "Submitted Successfully before " + str(days) + " days, " + str(hours) + " hours, " + str(
            ...
            message = "Submitted Successfully after " + str(days) + " days, " + str(hours) + " hours, " + str(
                minutes) + " minutes, " + str(seconds) + " seconds."
        return JsonResponse({
            'success': True,
            'message': message,
...
def evaluate_problem(request, sec_user_id, assign_id, prob_id):
    try:
        ...
        if UserSubmissions.objects.filter(user_id=request.session['user_id'], problem_id=prob_id).exists():
            final = UserSubmissions.objects.get(user_id=request.session['user_id'],
                                                problem_id=prob_id).final_submission_no
            contents = Submission.objects.get(user_id=request.session['user_id'], problem_id=prob_id, sub_no=final)
            problem = Problem.objects.get(problem_id=prob_id)
            ...
            if not compile(compile_cmd, work_dir):
                ...
                for t in testcases:
                    ...
                    if err == 0:
                        ...
                        marks = 0
                    s, created = SubTest.objects.update_or_create(testcase=t, sub=contents,
                                                                  defaults={'marks': marks, 'error': err,
...
def evaluate_all(request, sec_user_id, assign_id):
    cursor = connection.cursor()
    cursor.execute(
        '''select user_id,name from "user" where user_id in (select user_id from sec_user where sec_id in (select sec_id from sec_user where id=%s) and role='Student')''',
        [sec_user_id])
        ...
        for prob in problemArr:
            try:
                ...
                if UserSubmissions.objects.filter(user_id=user['user_id'], problem_id=prob_id).exists():
                    ...
                    work_dir = 'sedb/submissions/assign' + assign_id + '/prob' + str(prob_id) + '/sec_user' + str(
                        sec_user_id)
                    if not os.path.exists(work_dir):
                        os.makedirs(work_dir)
...
def delete_prob(request, sec_user_id, assign_id, prob_id):
    Problem.objects.filter(problem_id=prob_id).delete()
    return JsonResponse({
        'done': True
    })
